...
def DPSUR(dataset_name,train_dataset, test_data, model, batch_size, lr, momentum, epsilon_budget,delta, C_t, sigma_t,use_scattering,input_norm,bn_noise_multiplier,num_groups,bs_valid,C_v,beta,sigma_v,MIA,device,log_file):
    ...
    while epsilon<epsilon_budget:
        ...
        log_message(f'iters:{iter},epsilon:{epsilon:.4f} | Test set: Average loss: {test_loss:.4f}, Accuracy:({test_accuracy:.2f}%)')
