...
from train_and_validation.train_with_dp import train_with_dp, train_with_dp_agd, train_with_dp_agd_comp
from train_and_validation.validation import validation
from algorithm.CMDP_Mechanism import hyper_para_setup
from algorithm.CMDP_Mechanism import CMDPMechanism

def DPAGD_CMDP(train_data, test_data, model,optimizer, hyper_para, batch_size, epsilon_budget, C_for_loss, sigma_for_loss, device, log_file):
    ...
    while epsilon_con < epsilon_total:
        ...
        if iter == 1:
            cmdp = CMDPMechanism(epsilon=epsilon_used, delta2_f=optimizer.l2_norm_clip, batch_size=batch_size)
            optimizer.para_update(cmdp)

        train_loss, train_accuracy, RDP_con_iter1, lr_used = train_with_dp_agd_comp(model, train_dl, optimizer, epsilon_used, lr_used, hyper_para, C_for_loss, sigma_for_loss, batch_size / len(train_data), device)

        RDP_con_iter2 = optimizer.epsilon_R_subsampling(batch_size / len(train_data))

        RDP_con = RDP_con + RDP_con_iter1 + RDP_con_iter2
